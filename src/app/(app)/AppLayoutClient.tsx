...
import NovaShell from "@/components/themes/nova/NovaShell";
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Safety net: close the settings overlay whenever the route changes
  // (e.g. navigating to /developers from inside settings).
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const checkRedirect = () => {
      ...
      if (!userData && !hasData && !hasCreds) {
        if (pathname !== "/" && pathname !== "/privacy" && pathname !== "/terms" && pathname !== "/developers") {
          setIsRedirecting(true);
          router.replace("/login");
          return true;
        }
  ...
  return (
    <AppLayoutContext.Provider value={{ 
      ...
      <div className="h-full w-full bg-theme-bg overflow-hidden relative">
        {!userData && (pathname === "/" || pathname === "/privacy" || pathname === "/terms" || pathname === "/developers") ? (
