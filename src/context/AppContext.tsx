...
import { AcademiaData, ConnectionSource, YearDetection } from "@/types";
import { compareData, DataDiff } from "@/utils/shared/diffUtils";
import { sendNotification, syncNotificationData } from "@/utils/shared/notifs";
...
interface AppContextType {
  ...
  performLogin: (creds: any, source?: ConnectionSource) => Promise<any>;
  loginPromise: Promise<any> | null;
  setLoginPromise: (promise: Promise<any> | null) => void;
  logout: () => Promise<void>;
  disconnectAccount: () => Promise<void>;
  connectionSource: ConnectionSource | null;
  setConnectionSource: (source: ConnectionSource | null) => void;
  lastSyncAt: string | null;
  setLastSyncAt: (time: string | null) => void;
  connectedAt: string | null;
  setConnectedAt: (time: string | null) => void;
  academicYearLevel: number | null;
  setAcademicYearLevel: (year: number | null) => void;
  yearDetection: YearDetection | null;
  setYearDetection: (detection: YearDetection | null) => void;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [connectionSource, setConnectionSource] = useState<ConnectionSource | null>(() => {
    if (typeof window !== "undefined") {
      return (localStorage.getItem("classivo_connection_source") as ConnectionSource) || null;
    }
    return null;
  });
  const [lastSyncAt, setLastSyncAt] = useState<string | null>(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("classivo_last_sync_at") || null;
    }
    return null;
  });
  const [connectedAt, setConnectedAt] = useState<string | null>(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("classivo_connected_at") || null;
    }
    return null;
  });
  const [academicYearLevel, setAcademicYearLevel] = useState<number | null>(() => {
    if (typeof window !== "undefined") {
      const val = localStorage.getItem("classivo_academic_year");
      return val ? parseInt(val, 10) : null;
    }
    return null;
  });
  const [yearDetection, setYearDetection] = useState<YearDetection | null>(() => {
    if (typeof window !== "undefined") {
      const val = localStorage.getItem("classivo_year_detection");
      if (val) {
  ...
  // On mount: restore connection state from Supabase if localStorage was cleared
  useEffect(() => {
    const restoreConnectionState = async () => {
      // Only fetch from Supabase if we have userData but no local connection state
      if (!userData || connectionSource) return;

      const email = userData?.profile?.email;
      if (!email) return;

      try {
        const res = await fetchWithLoadBalancer(`/portal/status?username=${encodeURIComponent(email)}`);
        const data = await res.json();
        if (data.success && data.connected && data.source) {
          setConnectionSource(data.source as ConnectionSource);
          if (data.connected_at) {
            setConnectedAt(data.connected_at);
            localStorage.setItem("classivo_connected_at", data.connected_at);
          }
          if (data.last_sync_at) {
            setLastSyncAt(data.last_sync_at);
            localStorage.setItem("classivo_last_sync_at", data.last_sync_at);
          }
          localStorage.setItem("classivo_connection_source", data.source);
        }
      } catch (err) {
        // Silent fail — localStorage state is sufficient
      }
    };
  ...
  useEffect(() => {
    ...
    const checkClassNotifications = () => {
      ...
      const nextClassName = (status.nextClass as any).course || (status.nextClass as any).name || (status.nextClass as any).code || "Class";
  ...
  const logout = useCallback(async () => {
    ...
    setLastSyncAt(null);
    setConnectedAt(null);
    setAcademicYearLevel(null);
    setYearDetection(null);
    localStorage.removeItem("classivo_connection_source");
    localStorage.removeItem("classivo_last_sync_at");
    localStorage.removeItem("classivo_connected_at");
    localStorage.removeItem("classivo_academic_year");
    localStorage.removeItem("classivo_year_detection");
    router.replace("/login");
  }, [router]);

  const disconnectAccount = useCallback(async () => {
    // Notify backend to mark connection as inactive in Supabase
    try {
      const creds = EncryptionUtils.loadDecrypted("classivo_credentials");
      if (creds?.username) {
        await fetchWithLoadBalancer("/portal/disconnect", {
          method: "POST",
      ...
      // Backend disconnect is best-effort; continue with client cleanup
      console.error("Backend disconnect failed (non-critical):", err);
    }
  ...
  const performLogin = useCallback(async (creds: any, source: ConnectionSource = "academia") => {
    setIsBackendError(false);
    setBackendErrorMsg(null);
    const promise = (async () => {
      try {
        // Route to the appropriate backend endpoint based on connection source
        const endpoint = source === "srm_portal" ? "/portal/connect" : "/login";
        ...
        // Extract and persist academic year level
        if (data.academicYearLevel) {
          setAcademicYearLevel(data.academicYearLevel);
          localStorage.setItem("classivo_academic_year", String(data.academicYearLevel));
        }
        ...
        // Sync timetable + attendance to server for background push notifications
        setTimeout(() => syncNotificationData(data), 3000);
  ...
  const refreshData = useCallback(async (creds: any, existingData: any) => {
    ...
    try {
      ...
      localStorage.setItem("classivo_last_sync_at", syncTime);

      setUserData(mergedData);
      localStorage.setItem("classivo_data", JSON.stringify(mergedData));

      // Re-sync timetable + attendance to server for background push notifications
      setTimeout(() => syncNotificationData(mergedData), 2000);
  ...
  const value = useMemo(() => ({
    ...
    setProfileSeed,
    calendarData,
  }), [userData, customDisplayName, isUpdating, isOffline, isBackendError, backendErrorMsg, refreshData, performLogin, loginPromise, logout, disconnectAccount, connectionSource, setConnectionSource, lastSyncAt, setLastSyncAt, connectedAt, setConnectedAt, academicYearLevel, yearDetection, latestDiff, updateHistory, isUpdateHistoryOpen, deferredPrompt, canInstall, showWelcome, profileSeed, calendarData, isInitialized]);
