...
export default function Marks({
  ...
  // Target SGPA — persisted to localStorage via "Save Simulation"
  const [targetSgpa, setTargetSgpa] = useState<number>(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("classivo_target_sgpa");
      if (saved) {
  ...
  useEffect(() => {
    setMounted(true);
    // Load saved target SGPA
    const saved = localStorage.getItem("classivo_target_sgpa");
    if (saved) {
      const parsed = parseFloat(saved);
      if (!isNaN(parsed)) setTargetSgpa(parsed);
    } else if (data?.profile?.cgpa) {
      const profileCgpa = parseFloat(data.profile.cgpa);
      if (!isNaN(profileCgpa)) setTargetSgpa(profileCgpa);
    }
    // Load saved simulation marks
    const savedMarks = localStorage.getItem("classivo_simulated_expected_marks");
    if (savedMarks) {
      try {
        setExpectedMarksMap(JSON.parse(savedMarks));
      } catch {}
    }
  }, [data]);
  ...
  // Initialize expected marks to 60% of max external (more realistic default)
  useEffect(() => {
    if (subjects.length > 0 && Object.keys(expectedMarksMap).length === 0) {
      const initialMap: Record<number, number> = {};
      subjects.forEach((sub: any) => {
        const maxExt = 100 - sub.totalMax;
        initialMap[sub.id] = Math.round(maxExt * 0.6); // 60% realistic default
      });
  ...
  const targetGrades = useMemo(() => {
    ...
    subjects.forEach((sub: any) => {
      ...
      let gradeVal = 81; // Default to A+ (min threshold 81)
      if (total >= 91) gradeVal = 91;
      else if (total >= 81) gradeVal = 81;
      else if (total >= 71) gradeVal = 71;
      else if (total >= 61) gradeVal = 61;
      else if (total >= 56) gradeVal = 56;
      else if (total >= 50) gradeVal = 50;
      else gradeVal = 0;
    ...
    <div className="absolute inset-0 bg-[#0f131f] text-[#dfe1f4] overflow-hidden select-none font-body-lg">
      ...
      >
        <motion.main 
          ...
          animate="show"
          style={{ y: pullY }}
          className="pt-24 pb-36 px-5 max-w-4xl mx-auto space-y-8"
        >
          ...
          <motion.section variants={itemVariants} className="flex flex-col items-center justify-center py-4 shrink-0">
            <div className="relative w-64 h-64 flex items-center justify-center">
              <svg className="absolute inset-0 w-full h-full transform -rotate-90">
                {/* Base Track */}
                <circle className="opacity-30" cx="128" cy="128" fill="transparent" r={mainRadius} stroke="#262937" strokeWidth="12"></circle>
      ...
      {/* Floating Action Button — Opens SGPA Simulator */}
      <div className="fixed bottom-24 right-5 z-40">
        <motion.button
          whileTap={{ scale: 0.92 }}
          onClick={() => { Haptics.selection(); setShowSimulator(true); }}
          className="flex items-center gap-2 px-5 py-3.5 rounded-2xl font-label-caps text-[11px] font-black uppercase tracking-widest shadow-[0_4px_24px_rgba(110,231,247,0.3)] active:scale-95 transition-all border border-primary-container/30"
          style={{
            background: "linear-gradient(135deg, rgba(110,231,247,0.15) 0%, rgba(129,140,248,0.15) 100%)",
            backdropFilter: "blur(12px)",
          }}
        >
          <span className="material-symbols-outlined text-[18px] text-primary-container">calculate</span>
          <span className="text-primary-container">SGPA Simulator</span>
        </motion.button>
      </div>

      {/* SGPA Simulator Bottom Sheet */}
      <AnimatePresence>
        {showSimulator && (
          <>
            <motion.div
            ...
            >
              ...
              {/* Header */}
              <div className="flex items-center justify-between px-6 pt-3 pb-4 shrink-0 border-b border-outline-variant/10">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-body-sm text-[11px] text-on-surface-variant">adjust expected exam marks to project grades</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowSimulator(false)}
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar px-6 py-4 space-y-4">
                {subjects.map((sub: any) => {
                  ...
                  return (
                    <div key={sub.id} className="space-y-3 bg-slate-950/30 p-4 rounded-xl border border-outline-variant/10">
              ...
              {/* Footer — Simulated SGPA + Save */}
              <div className="px-6 py-5 border-t border-outline-variant/15 shrink-0 flex justify-between items-center bg-slate-950/40">
                <div>
                  ...
                  <span className="text-[30px] font-black text-primary-container" style={{ textShadow: "0 0 10px rgba(110,231,247,0.4)" }}>
                    {predictedGpa}
                  </span>
                </div>
                <button 
                  onClick={() => {
                    Haptics.heavy();
                    const newTarget = parseFloat(predictedGpa);
                    setTargetSgpa(newTarget);
                    localStorage.setItem("classivo_target_sgpa", newTarget.toString());
