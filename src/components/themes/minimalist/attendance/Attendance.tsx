...
function SubjectCard({ 
  ...
  const bunked = sub.conducted - sub.present;
  const toCompensate = Math.max(0, 3 * bunked - sub.present);
        ...
        {isExpanded && (
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3 mb-2">
              <div className="flex items-center gap-2 bg-surface-container-high/40 border border-outline-variant/10 rounded-xl p-3">
                <span className="material-symbols-outlined text-[18px] text-on-surface-variant">event_busy</span>
                <div>
                  <span className="text-[10px] text-on-surface-variant block uppercase font-bold leading-none mb-0.5">Bunked</span>
                  <span className="font-title-md text-[13px] text-on-surface font-black">{bunked} classes</span>
                </div>
              </div>
              <div className="flex items-center gap-2 bg-surface-container-high/40 border border-outline-variant/10 rounded-xl p-3">
                <span className="material-symbols-outlined text-[18px] text-primary-container">sync_saved_locally</span>
                <div>
                  <span className="text-[10px] text-on-surface-variant block uppercase font-bold leading-none mb-0.5">To Compensate (75%)</span>
                  <span className={`font-title-md text-[13px] font-black ${toCompensate > 0 ? "text-alert-rose" : "text-primary-container"}`}>
                    {toCompensate} classes
                  </span>
                </div>
              </div>
            </div>
      ...
      >
        ...
        >
          ...
          <motion.section variants={itemVariants} className="glass-panel border border-primary-container/20 rounded-2xl p-8 flex flex-col items-center text-center relative overflow-hidden shadow-xl">
            ...
            {/* Compensation Card */}
            <div className="glass-panel rounded-xl p-4 text-left border border-outline-variant/10 w-full">
              <span className="font-label-caps text-[10px] text-on-surface-variant block mb-1 font-bold">Overall to Compensate (75%)</span>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className={`material-symbols-outlined text-[18px] ${Math.max(0, 3 * stats.absent - (stats.conducted - stats.absent)) > 0 ? "text-alert-rose" : "text-primary-container"}`}>
                    sync_saved_locally
                  </span>
                  <span className={`font-title-md text-[18px] font-black ${Math.max(0, 3 * stats.absent - (stats.conducted - stats.absent)) > 0 ? "text-alert-rose" : "text-[#6ee7f7]"}`}>
                    {Math.max(0, 3 * stats.absent - (stats.conducted - stats.absent))} classes
                  </span>
                </div>
                <span className="text-[11px] text-on-surface-variant font-medium">
                  {Math.max(0, 3 * stats.absent - (stats.conducted - stats.absent)) > 0 
                    ? "attend these next to hit 75%" 
                    : "already safe & compensated"}
                </span>
              </div>
            </div>
          </motion.section>
