...
const MobileAttendance = ({
  ...
  const overallStats = useMemo(() => {
    if (baseAttendance.length === 0)
      return { pct: 0, badge: "safe", tagline: "all good", color: "text-[#ceff1c]", bunked: 0, compensate: 0 };
    ...
    const bunked = totalC - totalP;
    const compensate = Math.max(0, 3 * bunked - totalP);
    ...
    return { pct, badge, tagline, color, bunked, compensate };
  ...
  const { activeBunked, activeCompensate } = useMemo(() => {
    if (!activeSubject || activeSubject.conducted === undefined) {
      return { activeBunked: 0, activeCompensate: 0 };
    }
    
    let conducted = activeSubject.conducted;
    let present = activeSubject.present;
    
    if (isPredicting) {
      const imp = (predictionImpact && predictionImpact[activeSubject.id]) || { conducted: 0, present: 0 };
      conducted += imp.conducted;
      present += imp.present;
    }
    
    const bunked = conducted - present;
    const compensate = Math.max(0, 3 * bunked - present);
    return { activeBunked: bunked, activeCompensate: compensate };
  }, [activeSubject, isPredicting, predictionImpact]);
    ...
    <div className="h-full w-full flex flex-col bg-[#050505] text-white font-sans relative overflow-hidden touch-pan-y">
            ...
            >
              ...
              <div className="pb-1">
                ...
                <div className="mt-3 flex gap-2 flex-wrap">
                  <div className="bg-white/5 px-2.5 py-1 rounded-lg border border-white/5 flex items-center gap-1.5">
                    <span className="text-[9px] font-mono lowercase text-white/40">bunked:</span>
                    <span className="text-xs font-mono font-bold text-white">{activeBunked}</span>
                  </div>
                  <div className="bg-white/5 px-2.5 py-1 rounded-lg border border-white/5 flex items-center gap-1.5">
                    <span className="text-[9px] font-mono lowercase text-white/40">compensate:</span>
                    <span className={`text-xs font-mono font-bold ${activeCompensate > 0 ? "text-[#ff003c]" : "text-[#ceff1c]"}`}>
                      {activeCompensate} classes
                    </span>
                  </div>
                </div>
              </div>
      ...
      >
        <div className="px-6 flex flex-col gap-4 pt-4">
          <div className="sticky top-0 bg-[#f5f6fc] z-30 py-2 flex justify-between items-center border-b border-[#050505]/5 mb-2 w-full">
            <span className="font-mono text-[10px] lowercase tracking-widest text-[#050505]/40 font-bold">
              /// {isPredicting ? "predicted margin" : "watchlist"}
            </span>
            <div className="flex gap-3 text-[#050505]/60 font-mono text-[10px] lowercase font-bold">
              <span>
                bunked: <b className="text-black">{overallStats.bunked || 0}</b>
              </span>
              <span>
                compensate: <b className={(overallStats.compensate || 0) > 0 ? "text-[#ff003c]" : "text-black"}>{overallStats.compensate || 0}</b>
              </span>
            </div>
          </div>
